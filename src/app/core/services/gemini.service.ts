...
export class GeminiService {
  ...
  async sendMessage(message: string): Promise<string> {
    ...
    if (this.chatSession) {
      currentHistory = await this.chatSession.getHistory();
    }

    for (const modelName of this.FREE_MODELS) {
      try {
        if (!this.chatSession || this.currentModelName !== modelName) {
          const model = genAI.getGenerativeModel({ model: modelName });
          this.chatSession = model.startChat({ history: currentHistory });
